...
local function createTouchGuiContainer()
	if TouchGui then TouchGui:Remove() end
...
--				isJumpEnabled = true
		...
		setJumpModule(MasterControl.JumpEnabled)
		setClickToMove()
...
--				if isJumpEnabled then TouchJumpModule:Enable() end
	...
	MasterControl.Changed:connect(onControlsChanged)
...
--			setJumpModule(not IsModalEnabled)
			if IsModalEnabled then
				if CurrentControlModule then
					CurrentControlModule:Disable()
				end
			else
