...
local function isTree(part)
	return part:IsA('UnionOperation') and part.Anchored and part.Material == Enum.Material.Grass and (part.Size-Vector3.new(17.914, 15.394, 17.914)).magnitude < .2
end
...
local function PiercingCast(fromPoint, toPoint, ignoreList) --NOTE: Modifies ignoreList!
	repeat
		...
		if hitPart and (hitPart.Transparency > 0.95 or hitPart.CanCollide == false) and not isTree(hitPart) then
