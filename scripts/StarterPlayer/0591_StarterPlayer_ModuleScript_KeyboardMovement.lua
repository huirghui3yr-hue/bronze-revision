...
local PlayerIndoorsChangedCn = nil
...
function KeyboardMovement:Enable()
	...
	local leftArrowValue = 0
	local rightArrowValue = 0
	
	local updateMovement = function()
		MasterControl:AddToPlayerMovement(-currentMoveVector)
		local L = math.max(-1, leftValue + leftArrowValue)
		local R = math.min(1, rightValue + rightArrowValue)
		currentMoveVector = Vector3.new(L + R, 0, forwardValue + backwardValue)
		MasterControl:AddToPlayerMovement(currentMoveVector)
	end
	...
	local inputFunction = function(inputObject)
		local inputState = inputObject.UserInputState
		if inputObject.KeyCode == Enum.KeyCode.Left then
			if inputState == Enum.UserInputState.Begin and MasterControl.IsIndoors then
				leftArrowValue = -1
			elseif inputState == Enum.UserInputState.End then
				leftArrowValue = 0
			end
	...
	PlayerIndoorsChangedCn = MasterControl.Changed:connect(function(prop)
		if not MasterControl.IsIndoors then
			leftArrowValue = 0
			rightArrowValue = 0
			updateMovement()
		end
	...
	InputBeganCn = UserInputService.InputBegan:connect(inputFunction)
	InputEndedCn = UserInputService.InputEnded:connect(inputFunction)
...
function KeyboardMovement:Disable()
	...
	if PlayerIndoorsChangedCn then
		PlayerIndoorsChangedCn:disconnect()
		PlayerIndoorsChangedCn = nil
	end
