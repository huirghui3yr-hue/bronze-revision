			...
			spawn(function()
				...
				local bbg = create 'BillboardGui' {
					...
					create 'ImageLabel' {
						...
						Image = 'rbxassetid://5217879961',
						ImageColor3 = color,
						Size = UDim2.new(1.0, 0, 1.0, 0),
					},
					create 'ImageLabel' {
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://5217881600',
						Size = UDim2.new(1.0, 0, 1.0, 0),
						ZIndex = 2,
					}
				}
...
function Sprite:animCaptureAttempt(ballId, shakes, critical, caught)
	...
	delay(.5, function()
		Utilities.sound(300394723, nil, nil, 10)--pokemon return replace asap wdsfgrwefds2
	...
	for i = 1, shakes do
		Utilities.sound(301970857, 1, .1, 2) --ttr pokemon being caught replace asap
	...
	if caught then
		Utilities.sound(300394776, nil, nil, 5)--pokemon caught replace asap sdsvfgrwefds3
		for i = 1, 12 do
			_p.Particles:new {
				Position = cf.p,
...
--	print(spriteId, pokemon.forme)
	if (spriteId == 'Onix' or spriteId == 'Steelix') and self.forme == 'crystal' then
		customSparkle = function()
			Utilities.sound(282237234, nil, nil, 5)--shiny pokemon sfx replace asap
			local p = self.part
			local cf = p.CFrame
			for _ = 1, 8 do
				for _ = 1, 3 do
	...
	local function sparkle()
		...
		Utilities.sound(282237234, nil, nil, 5)--shiny pokemon sfx again replace asap
		local p = self.part
		for _ = 1, 8 do
			for _ = 1, 3 do
				local theta = math.random()*math.pi*2
...
--			wait(.15/speed)
	...
	delay(.8/speed, function() -- cry & ball sounds
		local v = .2
		local sound = Utilities.sound(300394663, v, nil, 5)--pokemon being thrown replace asap wadfsgdt1
		wait(.5/speed)
...
--		FormFactor = Enum.FormFactor.Custom,
	...
	Utilities.sound(300394866, nil, .2, 8)--i have no idea what this sound is but replace asap
