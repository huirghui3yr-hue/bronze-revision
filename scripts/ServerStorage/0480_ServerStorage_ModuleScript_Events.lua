...
--					_p.PlayerData.money = math.max(0, _p.PlayerData.money - 30000)
    ...
    onBeforeEnter_Lotto = function(room)
			local Prizes = game.ReplicatedStorage.Remote.GetLottoItems:InvokeServer(false)
			local NextPrizes = game.ReplicatedStorage.Remote.GetLottoItems:InvokeServer(true)
			local addItem = game.ReplicatedStorage.Remote.LottoItem
			local signs = room.model:WaitForChild('Signs1')
			local nextSigns = room.model:WaitForChild('Signs2')
			spawn(function()
				if Prizes then
					for i,v in pairs(Prizes) do
						signs[tostring(i)].SurfaceGui["1. stop"].Text = v or ''
						end
				end
				if NextPrizes then
					for i,v in pairs(NextPrizes) do
						nextSigns[tostring(i)].SurfaceGui["1. stop"].Text = v or ''
					end
				end
			end)
			local salesperson = room.npcs.pastelbranches
			interact[salesperson.model] = function()
				local HasTaken = game.ReplicatedStorage.Remote.HasDoneLottoToday:InvokeServer(false)
				if HasTaken == 'ready' then
					salesperson:Say('Hello! I am Pastel and this is my lotto,')
					if not Prizes then
						salesperson:Say('Unfortunatley, we dont give out tickets on saturdays.')
						salesperson:Say('Please come back later.')
						return
					end
					if salesperson:Say('[y/n]Would you like to claim today\'s ticket?') then
						if _p.PlayerData.badges[4] then
							if salesperson:Say('[y/n]You\'ll have to save before doing this, is this alright?') then
								local success = _p.PlayerData:save()
								if success then
									salesperson:Say('Save success!','Ok, so let\'s see that ticket!')									
									local num = math.random(1,5)
									local prize = Prizes[num]
									local s
									if prize ~= nil then
										print(prize)
										addItem:FireServer(prize)

									salesperson:Say('Oh wow! You won a '..prize..'!',
										'One '..prize..' has been added to your bag.'
										)
										local e = game.ReplicatedStorage.Remote.HasDoneLottoToday:InvokeServer(true)
										if e then
											salesperson:Say('Please come again in 24 hours!')
										else
											salesperson:Say('Don\'t come here again...')
										end
									end
									salesperson:Say('Have a nice day!')
								else
									salesperson:Say('Oh no, the save failed! you\'ll have to come back later.')
									return
								end
							else
								salesperson:Say('Ok then, have a nice day!')
								return
							end
						else
							salesperson:Say('You don\'t have the Anthian City gym badge, come back when you have that.')
							return
						end
					else
						salesperson:Say('Ok then, have a nice day!')
					...
					salesperson:Say('You already redeemed a ticket today.')
				end
			end
		end,
		...
		if completedEvents.MeetJake then
			...
			local function setupDevProduct()
				...
				interact[david.model] = function()
					spawn(function() _p.Menu:disable() end)
					local res = (function()
						if not chat:say(david, 'The professor sells these pokemon for 25 R$ each.', '[y/n]Would you like to purchase one?') then return end
...
--						end
			...
			local function setupJakeFight(jake, prof, mom, dad)
				...
				_p.DataManager:preload(303861020)
				touchEvent('JakeBattle1', room.model.CutsceneTrigger2, false, function()
					...
					jake:LookAt(p)
					chat:say(jake, 'We both have pokemon now, you know what that means?', 'LET\'S BATTLE!')
					...
					chat:say(jake, 'Nice fighting!', 'Here, allow me to heal your pokemon.')
					...
					Utilities.sound(288899943, nil, nil, 10)
					chat:say('Obtained 5 Poke Balls!', _p.PlayerData.trainerName .. ' put the Poke Balls in the Bag.')
					chat:say(jake, 'These Poke Balls can catch Pokemon!', 'It works best if you weaken the pokemon before throwing a Poke Ball at it.', 'Well, I need to gather a few things before I set off on my adventure.', 'You should go start training your pokemon on Route 1!', 'Past Route 1 is Cheshma Town.', 'That\'ll be a good place for us to meet up.', 'Alright, I\'ll see you later!')
					spawn(function()
						jake:WalkTo(exitp)
						jake:Remove()
					end)
				...
				if completedEvents.MeetParents then
					...
					touchEvent('ChooseFirstPokemon', room.model.CutsceneTrigger, false, function()
						...
						chat:say(prof, 'Yes, and there are still many things that we do not know about pokemon.',
							...
							'Make a choice from these 21 different breeds.',
							'Go ahead now.')
						...
						chat:say(prof, 'Also, if you\'d like, you can come back at any time and purchase another pokemon from my assistant, David.',
					...
					interact[room.model.David] = {'The professor sells these pokemon for 1 R$ each.',
	...
	onBeforeEnter_yourhomef1 = function(room)
		...
		onLoad_chunk2 = function(chunk)
			local function setupForestBattle()
				_p.DataManager:preload(311554660, 317469005)
		...
		onBeforeEnter_PokeCenter = function(room)
			...
			if npc then
				...
				interact[npc.model] = function()
					...
					if chat:say(npc, 'Hello, and welcome to the pokemon Center.', '[y/n]Would you like to rest your pokemon?') then
						spawn(function() chat:say(npc, '[ma]OK. I\'ll take your pokemon for a few seconds, then.') end)
						wait(1)
						npc:WalkTo(p2)
			...
			if martguy then
				interact[martguy.model] = function()
					spawn(function() _p.Menu:disable() end)
					chat:say(martguy, 'Welcome to the Poke Mart!', 'May I help you?')
					while true do
		...
		onBeforeEnter_EeveeHouse = function(room)
			local inGroup = _p.player:IsInGroup(10548213)
		...
		onBeforeEnter_Gym1 = function(room, continueCFrame)
			...
			_p.DataManager:preload(306160562,
				5120094157, 5226901509) -- vs text, trainer icon
			local m = room.model
		...
		onLoad_chunk7 = function(chunk)
			...
			if completedEvents.GroudonScene then
				...
				_p.DataManager:preload(317470444, 311554660, 317440901, 317447005,317447026, 68068592, 317480860) -- admin music, eclipse logo, earthquake, groudon scene music[2], particle, pulse
		...
		onBeforeEnter_Gym2 = function(room)
			_p.DataManager:preload(317011433, 317012607,
				5120094157, 496819113) -- vs text, trainer icon
		...
		onLoad_chunk8 = function(chunk)
			...
			if completedEvents.DamBusted then
				...
				local nw = chunk.map.NormalWater
				nw.Parent = nil
				local sig = Utilities.Signal()
				sceneSignal = sig
...
--[[					if #_p.PlayerData.party == 6 and not _p.PlayerData.pc:hasSpace() then  -- todo?
		...
		onLoad_chunk11 = function(chunk)
			...
			interact[defaultio.model] = function()
				if completedEvents.GetGoodRod then
					defaultio:Say('Fishing with the Good Rod will allow you to catch some pokemon that are only obtainable with it.')
					...
					chat:say(defaultio, 'You\'ll find higher-level pokemon than with an Old Rod.',
						'There are even some pokemon you can catch only with a Good Rod.')
				end
			end
			...
			if completedEvents.LighthouseScene then
				...
				connectLighthouseScene = function()
					...
					touchEvent('LighthouseScene', lhtrigger, false, function()
						...
						_p.DataManager:preload(337448917, 243728104, 337594642, 337448867, 6223017600, 317480860, 338262406)
						_p.DataManager:preloadModule('AnchoredRig')
						...
						delay(91, function()
							if not sceneMusic then return end
							sceneMusic = Utilities.loopSound(337448867, .75)
						end)
						local rain
						delay(1, function()
						...
						* CFrame.new(0, 0, -15) * CFrame.Angles(-0.15, 0, 0)
						...
						spawn(function() admin:LookAt(grp) end)
						admin:Say('Quickly, hand me an empty Poke Ball.')
						spawn(function() man:LookAt(kyp) end)
						admin:LookAt(kyp)
		...
		onBeforeEnter_Gym3 = function(room, continueCFrame)
			...
			local m = room.model
			_p.DataManager:preload(5120094157, 5226490625) -- vs text, trainer icon
			-- continue support
			if continueCFrame then
				continueCFrame = continueCFrame + m.Base.Position
				for i = 3, 1, -1 do
		...
		onLoad_chunk14 = function(chunk)
			...
			if not completedEvents.Jirachi then
				_p.DataManager:preload(380888410, 380888758)
				...
				chatCn = _p.player.Chatted:connect(function(msg)
					...
					if MasterControl.WalkEnabled and not _p.DataManager.isDay and msg:lower():match('^i%s+wish%s+%a') then
						...
						if pos.Y > 0 and pos.Y < 5 and math.sqrt(pos.X^2+pos.Z^2) < 3.5 then
							...
							jirachi.Parent = map
							_p.MusicManager:stackMusic(380888410, 'Jirachi')
						...
						--]]
							...
							_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Wish,{battleSceneType = 'Jirachi', musicId = 380888758})
				...
				if completedEvents.BlimpwJT then
					...
					touchEvent('BlimpwJT', map.BlimpButton, true, function()
						...
						tess:Say('His name is Gerald.',
							...
							'I haven\'t seen him for a while, but I know he works at the Poke Ball shop in Anthian City\'s Shopping District.',
...
--				blimpEngine.Anchored = true
			...
			--]]
					...
					if d.firstTime then
						...
						tess:Say('Well, Anthian City is divided into four districts',
							...
							'The Shopping District is where you can do a lot of shopping for Poke Balls, clothes and other neat items.',
	...
	--]]		
					...
					if not completedEvents.MeetGerald then
						...
						touchEvent('MeetGerald', map.JTCutscene, false, function()
							...
							tess:Say('Welcome to the Shopping District, '.._p.PlayerData.trainerName..'.',
								'The Poke Ball shop is just ahead.',
								'Let\'s all go now.')
							...
							local path = {
								...
								Vector3.new(-269.2, 64, 588),
								cf * Vector3.new(0, 0, 10),
							}
							local walking = true
							spawn(function()
				...
				onBeforeEnter_PokeBallShop = function(room)
					...
					interact[salesperson.model] = function()
						spawn(function() _p.Menu:disable() end)
						chat:say(salesperson, 'Welcome to the Poke Ball Emporium!', 'May I help you?')
						while true do
				...
				onBeforeEnter_HerosHoverboards = function(room)
					...
					local function onClickHoverboard(model)
						...
						if model.Name:sub(1, 6) == 'Basic ' then
							...
							if _p.Network:get('PDS', 'ownsHoverboard', model.Name) then
								...
								if shopGuy:Say('[y/n]Ah, '..model.Name..'... Would you like to purchase this one for 50 R$?')
					...
					interact[salesperson.model] = function()
						...
						elseif choice == 2 then
							...
							salesperson:Say('This is our Deluxe Collection. Once you purchase a board for 50 R$ you can take it out any time!',
								'Click on whichever one you\'d like!')
				...
				onBeforeEnter_RorianBraviary = function(room)
					...
					interact[salesperson.model] = function()
						...
						if c == 1 then
							...
							Utilities.Write 'Rorian Braviary is a third-party clothing company and is not affiliated with pokemon Brick Bronze.' {
								Frame = textbox,
				...
				onBeforeEnter_ApartmentHigh = function(room)
					...
					local receptionist = room.npcs.Receptionist
					interact[receptionist.model] = {'Welcome to the Golden Poke Ball.',
				...
				onBeforeEnter_Gym4 = function(room)
					...
					_p.DataManager:preload(5120094157, 496819222) -- vs text, trainer icon
					-- tools
				...
				onLoad_chunk22 = function(chunk, data)
					...
					if _p.PlayerData.badges[4] and not completedEvents.DefeatTEinAC then
						...
						spawn(function()
							...
							_p.MusicManager:stackMusic(316929620, 'Cutscene', .4)
							Utilities.FadeIn(1)
							...
							tess:Say('No, please stop!', 'Think of all the people and pokemon that will be Removeed as a result of this!')
							...
							-- Dialga and Palkia
							_p.MusicManager:stackMusic(510817248, 'Cutscene')
							--				wait(.75)
							...
							local dCry = {id = 301816205, duration = 1.57, startTime = 2.21}
							local pCry = {id = 301816205, duration = 1.63, startTime = 4.79}
...
--[[		local lawyer = chunk.npcs.EventLawyer
		interact[lawyer.model] = function()
			...
			elseif completedEvents.BeatSanta then
					...
					elseif completedEvents.EnteredPast then
						...
						touchEvent(nil, model.TylerTrigger, nil, function()
							...
							gerald:Say('That is a wonderful idea.', 'Tess and I will be searching hard for critical details.',
								...
								'If you want to talk again before you leave, you can find me at the Poke Ball shop.')
							spawn(function()
				...
				onLoad_chunk25 = function(chunk)
					...
					interact[sanjay.model] = function()
						if completedEvents.GiveEkans then
							sanjay:Say('Thanks again for the Ekans!', 'I hope you are enjoying the Poke Flute!')
						else
							if sanjay:Say('I am an aspiring Ekans Charmer.',
								...
								'I have an extra Poke Flute that I would trade for it.',
								...
								if not tradeslot or not d then
									...
									local trade = sanjay:Say('[y/n]Are you sure you want to trade this Ekans for my Poke Flute?')
									...
									if not trade then
										...
										onObtainKeyItemSound()
										chat:say('Obtained a Poke Flute!', _p.PlayerData.trainerName .. ' put the Poke Flute in the Bag.')
									end
								end
							end
						end
				...
				onBeforeEnter_PBStampShop = function(room)
					...
					interact[guy.model] = function()
						spawn(function() _p.Menu:disable() end)
						guy:Say('Welcome to the Poke Ball Stamp Shop!')
						if completedEvents.PBSIntro then
							...
							guy:Say('Here you can try your luck on the Stamp Spinner and see which stamps you\'ll win.',
								'Poke Ball Stamps can be applied to your Poke Balls to produce cool effects as you send your pokemon into battle!',
								'I am here to assist you with using the Stamp Spinner.',
								'The lady to your right will assist you with applying stamps from your inventory to your Poke Balls.',
					...
					interact[gal.model] = function()
						spawn(function() _p.Menu:disable() end)
						gal:Say('Welcome to the Poke Ball Stamp Shop!')
						...
						if chat:choose('Customize', 'Cancel') == 1 then
							gal:Say('Which pokemon\'s Poke Ball would you like to customize?')
							local slot = _p.BattleGui:choosePokemon('Choose')
							if slot then
						...
						touchEvent(nil, map.CTrigger, false, function()
							...
							* CFrame.Angles(.1, 0, 0) -- + Vector3.new(0, 2, 0)
							...
							tess:Say('We will never join the likes of you.',
								'You people will Remove this world and see a new world fail as people and Pokemon stop helping each other to prosper.',
							...
							* CFrame.Angles(.15, 0, 0) + jake.model.HumanoidRootPart.Position
							...
							_p.MusicManager:stackMusic(667228671, 'Cutscene')
							wait(.5)
							local speed = .5
							local function schedule(animTrack, kfName, kfTime, func)
								local fired = false
		...
		local map = chunk.map
		local latios, latias = map:FindFirstChild('Latios'), map:FindFirstChild("Latias")
		if completedEvents.LatiosLatias then
			latios:Remove()
			latias:Remove()
		else		
		touchEvent('LatiosLatias', map.LatiosLatiasTE, true, function() --LatiosLatiasTE (old event name)
				MasterControl.WalkEnabled = false
				_p.RunningShoes:disable()
				Utilities.exclaim(_p.player.Character.Head)
				local walking = true
				spawn(function()
				MasterControl:WalkTo(CFrame.new(-1789.57104, 2626.69458, -1659.03589, 0.632368088, 0, -0.774668097, 0, 1, 0, 0.774668097, 0, 0.632368088)) 
					walking = false
				end)
					wait(.5)
local sound = Instance.new("Sound", workspace)
sound.SoundId = "rbxassetid://725512657"
sound.Playing = true
sound.Volume = 1.5
wait(.3)
--Cam Tween Stat
  ...
  local goals = {
   CFrame = CFrame.new(-1791.27087, 2630.09985, -1652.63171, 0.668361008, -0.135289863, -0.731430292, -0, 0.983320534, -0.18188104, 0.743837059, 0.121562198, 0.657213092)}
  local tweenc = tween:Create(cam,tweeninfo,goals)
  tweenc:Play()
wait(.5)
--Latios Tween 1
local tween = game:GetService("TweenService")
local part = map.Latias.Part
local tweeninfo = TweenInfo.new(2.2,Enum.EasingStyle.Linear,Enum.EasingDirection.In,0,false,0)
local goals = {
 CFrame = CFrame.new(-1685.39746, 2648.78149, -1582.81348, 0.0141479978, 0.790911913, 0.611766458, -0.0182809979, 0.611930072, -0.790700614, -0.999732852, 3.12933889e-06, 0.0231162403)
}

local tweenc = tween:Create(part,tweeninfo,goals)
tweenc:Play()
					
						
--Latias Tween 1
  local tween = game:GetService("TweenService")
  local part = map.Latios.Part
  local tweeninfo = TweenInfo.new(2.2,Enum.EasingStyle.Linear,Enum.EasingDirection.In,0,false,0)
  local goals = {
   CFrame = CFrame.new(-1683.59753, 2639.60718, -1577.76624, 0.0141479978, 0.790911913, 0.611766458, -0.0182809979, 0.611930072, -0.790700614, -0.999732852, 3.12933889e-06, 0.0231162403)
  }
...
--Cam Tween 1
  ...
  local tweeninfo = TweenInfo.new(2.2,Enum.EasingStyle.Linear,Enum.EasingDirection.In,0,false,0)
  local goals = {
   CFrame = CFrame.new(-1792.64465, 2630.27124, -1656.84827, -0.6112656, -0.0912017897, -0.786153078, -3.72529074e-09, 0.993338108, -0.11523734, 0.791425645, -0.0704406202, -0.607193351)}
  local tweenc = tween:Create(cam,tweeninfo,goals)
  tweenc:Play()
  spawn(function() MasterControl:LookAt(map.LookAt.CFrame) end)
   wait(2.2)
					latios:Remove()
					latias:Remove()
					local latios2, latias2 = map:FindFirstChild("Latios2"), map:FindFirstChild("Latias2")
					
					--Latios Tween 2
local tween = game:GetService("TweenService")
local part = map.Latios2.Part
local tweeninfo = TweenInfo.new(2.2,Enum.EasingStyle.Linear,Enum.EasingDirection.In,0,false,0)
local goals = {
 CFrame = CFrame.new(-1918.1759, 2820.91626, -1785.22974, 0.627210677, 0.481868565, -0.611890078, -0.527711749, 0.840737641, 0.121163212, 0.572823703, 0.246906698, 0.781607389)
}
...
--Latias Tween 2
  local tween = game:GetService("TweenService")
  local part = map.Latias2.Part
  local tweeninfo = TweenInfo.new(2.2,Enum.EasingStyle.Linear,Enum.EasingDirection.In,0,false,0)
  local goals = {
   CFrame = CFrame.new(-1885.70361, 2809.79053, -1773.44165, 0.61432302, 0.494282752, -0.615054369, -0.551941991, 0.826230764, 0.112707019, 0.563885987, 0.270235837, 0.780387878)
  }
...
--Cam Tween 2
  ...
  local tweeninfo = TweenInfo.new(2.2,Enum.EasingStyle.Linear,Enum.EasingDirection.In,0,false,0)
  local goals = {
   CFrame = CFrame.new(-1790.98157, 2628.88379, -1654.46216, 0.99701041, 0.0625164509, 0.0454101898, -3.72529074e-09, 0.587694943, -0.809082627, -0.0772683099, 0.806663811, 0.585937917)}
  local tweenc = tween:Create(cam,tweeninfo,goals)
  tweenc:Play()
 spawn(function() MasterControl:LookAt(map.LookAt2.CFrame)
wait(2.2)
				...
				latios2:Remove()
				latias2:Remove()	
				chat:say('The wild pokemon fled!', 'Latios and Latias can now be found roaming in the wild.')
				spawn(function() _p.PlayerData:completeEvent('LatiosLatias') end)
						...
						if _p.PlayerData.badges[7] then
							...
							if win then
								Zeek:Say('Well, I guess you win fair and square.',
									'I must say i wasn\'t expecting your Pokemon to handle this so well.',
								...
								Zeek:Say('Equipped with the Contrast Badge you will be able to trade for Pokemon up to level 90.',
...
--[[					local Tess = chunk.npcs.Tess
					if _p.PlayerData.completedEvents.TessBattle then
						...
						Tess:Say('Hey!', 'Before we move onto Port Decca you should probably battle the Frostveil City gym!', 'I heard it\'s a dark type gym.')
					    else
						_p.Menu:disable()
						Tess:Say('Oh you did it!',
							'You beat Frostveil\'s gym leader! ', 'That\'s really impressive!', 'Listen,' .. _p.PlayerData.trainerName .. 'I have a rquest before we move onto Port Decca.', 'It\'s been a while since we battled each other ',
							'I\'ve also been training my Pokemon, and I think I\'ve gotten a lot stronger ', 'What I\m saying is, would you mind if we had another battle? ', 'I\ll try my best, and put my all into it!')
						local win = _p.Battle:doTrainerBattle {
							musicId = 494373237,
							trainerModel = Tess.model,
							vs = {name = 'Tess', id = 0, hue = 0.467, sat = .2, val = .7},
							num = 185
						}        
						if win then
							_p.PlayerData.completedEvents.TessBattle = true
							Tess:Say('Wow! You\'ve gotten so much stronger!',
								'Thanks for the battle.', 'Route 16 is right through here, then we can go onto Cosmeos Valley and eventually Port Decca', 'Over there we can take a boat to Crescent Town and get your parents back!', 'Let\'s go!')
							Utilities.FadeOut(.25)
							Tess:Remove()
							Utilities.FadeIn(.25)
						end
						MasterControl.WalkEnabled = true
		...
		--[[
			onLoad_chunk51 = function(chunk) -- cosmeos valley
		if not completedEvents.vVictoryRoad then spawn(function() _p.PlayerData:completeEvent('vVictoryRoad') end)end
		...
		if _p.PlayerData.badges[9] then --need victory road badge
		interact[leader.model] = postWinInteract
		else
		...
		chat:say(GateKeeper, 'Brimber City should be just through this gate.',
			'They have a Pokemon Gym there too.',
			'We should also ask around and see if anyone knows anything about Team Eclipse while we\'re here.',
			'I\'m gonna head over to the pokemon Center and heal my pokemon!',
			'See you later!')
		...
		GateKeeper:WalkTo(door.Position)
		spawn(function() GateKeeper:WalkTo(door.Position + Vector3.new(0, 0, 20)) end)
	...
	--]]	
		...
		onLoad_chunk52 = function(chunk)
			if not completedEvents.vPortDecca then
				spawn(function()
					_p.PlayerData:completeEvent("vPortDecca")
				end)
			end
-- [Old]	if not completedEvents.vPortDecca then spawn(function() _p.PlayerData:completeEvent('vPortDecca') end)end
			local capn = chunk.npcs.Captain
				...
				do
					...
					if #dests == 0 then
						...
						if choice == #options then
							...
							chunk:Remove()
							wait()
							chunk = _p.DataManager:loadChunk(dest.chunk)
	    ...
	    onExit_Gate22 = function()
      local chunk = _p.DataManager.currentChunk
      if chunk.id ~= "chunk52" then
        return
      end
      if completedEvents.meetBradv5 then
        return
      end
      spawn(function()
        _p.PlayerData:completeEvent("meetBradv5")
      end)
      local brad = chunk.npcs.Brad
      local tess = chunk.npcs.Tess
      local bp = Vector3.new(-950.343, 2946.657, 1481.648)
      local tp = Vector3.new(-944.636, 2946.68, 1481.572)
      local pp = Vector3.new(-947.641, 2944.25, 1485.944)
      brad:Teleport(CFrame.new(bp, tp))
      tess:Teleport(CFrame.new(tp, bp))
      local narhat = create("Part")({
        CanCollide = false,
        Size = Vector3.new(1, 1, 1),
        Parent = brad.model,
        create("SpecialMesh")({
          MeshType = Enum.MeshType.FileMesh,
          MeshId = "rbxassetid://118203206",
          TextureId = "rbxassetid://118203087"
        })
      })
      local rs = brad.model.Torso["Right Shoulder"]
      rs.C1 = rs.C1 * CFrame.Angles(0.4, 0, 0)
      local ra = brad.model["Right Arm"]
      create("Weld")({
        Part0 = ra,
        Part1 = narhat,
        C0 = CFrame.new(-0.9, -1.1, 0) * CFrame.Angles(0, 0, -0.4),
        C1 = CFrame.new(),
        Parent = ra
      })
      local cam = workspace.CurrentCamera
      cam.CameraType = Enum.CameraType.Scriptable
      cam.CFrame = CFrame.new(Vector3.new(-947.641, 2950.694, 1489.105), (tp + bp) / 2 + Vector3.new(0, 1.5, 0))
      local tname = _p.PlayerData.trainerName
      _p.RunningShoes:disable()
      spawn(function()
        MasterControl:WalkTo(pp)
      end)
      ...
      tess:LookAt(pp)
      tess:Say("Oh good, you made it, " .. tname .. ".")
      brad:Say("Oh, so this is " .. tname .. ".", "Your friend here was just telling me about your situation.", "It's quite unfortunate what has happened to you, but it sounds like you've come a long way since.")
      tess:Say("Oh right, " .. tname .. ", this is Brad.", "Brad was just telling me that he had just come from Crescent Island.", "Apparently it's not a very friendly place.")
      ...
      brad:Say("It's quite an interesting place, and it wouldn't surprise me if their headquarters was on the island.", "I wasn't there to associate with the community, though.", "I was there to earn my final gym badge so that I could challenge the Roria League.", "You need eight gym badges to compete, and it so happens that the the last badge I needed is found on that island.")
      spawn(function()
        brad:LookAt(tp)
      end)
      tess:Say("Oh, that's interesting to know!", tname .. " has been collecting Roria's badges and only needs one more of the eight.")
      brad:LookAt(pp)
      brad:Say("Is that so?", "Well, if you do earn the badge on the island, maybe we'll get a chance to compete at the league.", "I won't give away any details on the gym there, but it wasn't an easy battle.")
      spawn(function()
        brad:LookAt(tp)
      end)
      tess:Say("That shouldn't be a problem.", tname .. " is one of the toughest trainers in Roria.")
      brad:LookAt(pp)
      brad:Say("Oh really?", "Well then, one day we ought to get a chance to battle.", "Oh, speaking of Crescent Island, though...", "Not even the burliest of the sailors around here will take you there on their boat.", "Instead you'll need to travel across the waters on Route 17.", "You'll need this. Please take it, I insist!")
      chat.bottom = true
      onObtainItemSound()
      chat:say("Obtained an HM03!", tname .. " put the HM03 in the Bag.")
      chat.bottom = nil
      brad:Say("It's a Hidden Machine that contains the move Surf.", "Surf will enable you to ride your pokemon across the water.", "You'll need it to reach Crescent Island.")
      spawn(function()
        brad:LookAt(tp)
      end)
      ...
      tess:Say("What a nice guy.", "Anyways, now's our chance to get ready to head out for Crescent Island.", "Once we get there we'll need to find Team Eclipse's secret base, so we'll probably want to be ready to fight.", "I'm going to run into the Pokemon Center and prepare for the journey ahead.", "When you're ready, meet me at Decca Beach!", "I'll see you there, " .. tname .. "!")
      tess:WalkTo(Vector3.new(-947.433, 2946.071, 1447.623))
      local door = chunk:getDoor("PokeCenter")
      spawn(function()
        door:open(0.7)
      end)
      tess:WalkTo(Vector3.new(-947.175, 2944.271, 1441.308))
      door:close(0.5)
      tess:Remove()
      _p.Menu:enable()
      _p.RunningShoes:enable()
      return true
		...
		onLoad_LostIslands = function(chunk)
			local Capn = chunk.npcs.Captain
			interact[Capn.model] = function()
				if not Capn:Say('[y/n]Ahoy! Are ye ready for yer return voyage to Port Decca?') then
					Capn:Say('Blimey! Another time, then?')
					return
				end
				Capn:Say('All aboard!')
				...
				Utilities.TeleportToSpawnBox()
				chunk:Remove()
				-- Change to Port Decca chunk
				_p.DataManager:loadChunk('chunk52')

				workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
				Utilities.Teleport(CFrame.new(-918, 2928.709, 1274))
				local elapsed = tick()-startTick
				if elapsed < .5 then
					wait(.5-elapsed)
				end
		...
		onBeforeEnter_Agency = function(room)
        local saleslady = room.npcs.SalesLady
        interact[saleslady.model] = function()
            spawn(function() _p.Menu:disable() end)
            chat:say(saleslady, 'Welcome to the Port Decca travel agency!', 'You can select from available tickets to travel to other places off the shores of Roria!', 'Have a look at our avaliable tickets.')
            while true do
                if not _p.Menu.shop:open('shiptix') then break end
                chat:say(saleslady, 'Is there anything else I may do for you?')
            end
            chat:say(saleslady, 'Thanks, come back and check with us again!')
            _p.Menu:enable()
        end
    end,		
				
				
				--[[onLoad_chunk53 = function(chunk) -- gym 7 is also a chunk
					  local Cptn = chunk.npcs.Captain
					interact[Cptn.model] = function()
						if not Cptn:Say('Im giving free rides, since surf is currently blocked off',
							'[y/n]Want to teleport to Cresent Island?') then
							Cptn:Say('Come back if you wanna go.')
							return
						end
						Cptn:Say('Alright, let\'s Go!')
						Utilities.FadeOut(.5)
						local startTick = tick()
						Utilities.TeleportToSpawnBox()
						chunk:Remove()
						-- Change chunks to Crescent Town
						_p.DataManager:loadChunk('chunk54')
						
						workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
						Utilities.Teleport(CFrame.new(-867.097, 464.31, -836.944))
						local elapsed = tick()-startTick
						if elapsed < .5 then
							wait(.5-elapsed)
						end
		...
		local Tess = chunk.npcs.Tess
		local pName = _p.PlayerData.trainerName
		local fishman = chunk.npcs.fishman


			if completedEvents.JakeAtDB then
				map.Tess:Remove()
				map.Boat:Remove() 
				map.fishman:Remove() 
		else
				touchEvent('JakeAtDB', map.DeccaBeachTE, true, function()
					MasterControl:Stop()
					spawn(function() _p.Menu:disable()end) 
					_p.RunningShoes:disable()
					Tess:LookAt(humroot.Position)
					Utilities.exclaim(Tess.model.Head)
					local wave = Tess.humanoid:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.NPCWave })
					wave:Play()
					MasterControl:WalkTo(Vector3.new(306.289, 424.063, -304.038))
					Tess:Say('I\'ve got good news ' ..  pName  .. '!', 'The older couple that run the fish market here said that we can try to catch a ride to Crescent Island with one of their fisherman!',
					'He rides all the way out to Crescent Island to gather fish for their market.', 'He\'s down at the beach right now so we\'ll need to catch him quickly.')
					local cords = {
					Vector3.new(292.564, 414.273, -265.098),
					Vector3.new(207.195, 401.785, -210.96)
					
					}
					local pcords = {
					Vector3.new(299.024, 414.273, -265.088),
					Vector3.new(209.53, 401.899, -207.747)
					
					}
					
					spawn(function()
					for i, p in pairs(cords) do
						Tess:WalkTo(p)
					end
					end)
					wait(.4)
					spawn(function()
					for i = 1, 2 do
					MasterControl:WalkTo(pcords[i])
					end
					end)
					wait(10)
					Tess:Say('Excuse me sir!')
					MasterControl:LookAt(workspace.chunk53.fishman.HumanoidRootPart.Position)
					fishman:LookAt(workspace.chunk53.Tess.Head.Position)
					fishman:Say('Aye, if you want some of Kade Krabby\'s Karefully-Krafted Klassic Kantonian Kwik-Krisped Magikarp, I just gave all I caught to my boss at the market')
					...
					fishman:Say('Oh, well alright, what is it?')
					Tess:Say('Well my name is Tess, and this is my friend, ' ..  pName  .. ' and we need to get to Crescent Island', 
					'I talked to the older couple at the market and they told me that you make fishing trips out to Crescent Island frequently.', 'So I was wondering...')
					fishman:Say('Stop it right there missy! Do you know what kinda people are out on that island?', 'They\'re some of the roughest, toughest, nastiest scoundrels you\'ll ever meet...')
					fishman:LookAt(humroot.Position)
					wait(.2)
					fishman:LookAt(workspace.chunk53.Tess.Head.Position)
					fishman:Say('You kids aren\'t serious about going there, are you?')
					Tess:Say('Yes, we are on a mission to save friends and family.', 'We\'re chasing a group of goons that go by the name "Team Eclipse".',
					'They are plotting something disastrous, and if we dont stop them, Roria could be in danger.')
					fishman:Say('Wow that is quite a tale.', 'I\'ve hear a lot of stories in my time, and yours sounds just as fishy as a fishing story.'
					,'Still, both of you look rather determined to get there, regardless of your reasons...')
					Tess:Say('I wish I were making this up...', 'We\'ll do anything to get to that island.')
					fishman:Say('Is that so?', 'Well I suppose our ony obstacle now is that I can really only take one extra person in my boat.')
					Tess:LookAt(humroot.Position)
					MasterControl:LookAt(workspace.chunk53.Tess.Head.Position)
					Tess:Say('Ok I have a new plan.', "I saw someone earilr, looked like a captain or pirate." 
					,'I will ride in the boat with this guy to the island, and you can take that captain or whatever is there', 'I know you, and I know you can handle the pirate if he tries something'
				,'And don\'t worry about me getting on a boat with a stranger. Garchomp can probably take this guy, easily. I mean, just look at him. His best move is probably struggle...'
				,'Alright it\'s settled' ,'I\'ll ride in the boat with you, and ' .. pName .. ' will teleport to Crescent Island.')
				fishman:Say('Aight, if I can\'t talk you out of it, I would at least recommend stocking up on items and healing your Pokemon before we depart.')
				Tess:LookAt(humroot.Position)
				Tess:Say('I already just healed my team!' ,'Make sure you do the same', 'Come find me when you get to Crescent Island!'
				,'Alright I\'m ready to go!')
				Tess:LookAt(workspace.chunk53.fishman.HumanoidRootPart.Position)
				fishman:Say('Alright, hop in and we\'ll be off')
				local tween = game:GetService("TweenService")
				local info1 = TweenInfo.new(.5,Enum.EasingStyle.Cubic,Enum.EasingDirection.Out,0,false,0)
				local goals1 = {CFrame = CFrame.new(198.0867, 411.13504, -220.413651, 0.0926529542, 0.000773628708, 0.995698214, 0, 0.999999762, -0.000776970817, -0.995698512, 7.19886448e-05, 0.0926529244)}
				local goals2 = {CFrame = CFrame.new(197.280777, 411.13504, -211.752686, 0.0926529542, 0.000773628708, 0.995698214, 0, 0.999999762, -0.000776970817, -0.995698512, 7.19886448e-05, 0.0926529244)}
                local twc1 = tween:Create(game.Workspace.chunk53.fishman.HumanoidRootPart,info1,goals2)
                local twc2 = tween:Create(game.Workspace.chunk53.Tess.HumanoidRootPart,info1,goals1)
                twc1:Play()
                twc2:Play()
wait(.55)
               local goals3 = {CFrame = CFrame.new(189.982285, 401.832001, -218.847031, 0.849015474, -0.00369389821, 0.528354883, 0.00926895253, 0.999925673, -0.00790351816, -0.528286397, 0.0116075035, 0.848986864)}
				local goals4 = {CFrame = CFrame.new(192.44664, 401.795135, -214.887177, 0.849015474, -0.00369389821, 0.528354883, 0.00926895253, 0.999925673, -0.00790351816, -0.528286397, 0.0116075035, 0.848986864)}
                 local twc3 = tween:Create(game.Workspace.chunk53.fishman.HumanoidRootPart,info1,goals3)
                local twc4 = tween:Create(game.Workspace.chunk53.Tess.HumanoidRootPart,info1,goals4)
                 twc3:Play()
                twc4:Play()
game.Workspace.chunk53.fishman.HumanoidRootPart.CFrame =  CFrame.new(189.982285, 401.832001, -218.847031, 0.849015474, -0.00369389821, 0.528354883, 0.00926895253, 0.999925673, -0.00790351816, -0.528286397, 0.0116075035, 0.848986864)
game.Workspace.chunk53.Tess.HumanoidRootPart.CFrame =  CFrame.new(192.44664, 401.795135, -214.887177, 0.849015474, -0.00369389821, 0.528354883, 0.00926895253, 0.999925673, -0.00790351816, -0.528286397, 0.0116075035, 0.848986864)
				spawn(function()_p.Menu:enable()end)
				completedEvents.JakeAtDB = true
                MasterControl.WalkEnabled = true
_p.RunningShoes:enable()
				end)
			end
                ...
                map.Gym8Blocker:Remove()   
                end      
					if not completedEvents.vCrescentIsland then spawn(function() _p.PlayerData:completeEvent('vCrescentIsland') end)
					--[[	local Cptn = chunk.npcs.Captain
						interact[Cptn.model] = function()
							if not Cptn:Say('[y/n] Are ye ready for yer return voyage to Decca Beach') then
								Cptn:Say('Come back when yer ready.')
								return
							end
							Cptn:Say('All aboard!')
							Utilities.FadeOut(.5)
							...
							Utilities.TeleportToSpawnBox()
							chunk:Remove()
							-- Change chunks to Decca Beach, I note this so I don't get confused xD
							_p.DataManager:loadChunk('chunk53')
							
							workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
							Utilities.Teleport(CFrame.new(221, 402.014, -329))
							local elapsed = tick()-startTick
							if elapsed < .5 then
								wait(.5-elapsed)
							end
				...
				onLoad_gym8 = function(chunk)
					...
					interact[Captain.model] = function()
						if _p.PlayerData.badges[8] then
							...
							Captain:Say("Hehehe, did that scare you?",
							...
							if win then
								Captain:Say("A pirate always has a hard time parting ways with their treasure, but I cannot deny someone who has earned one of these.",
								...
								chat.bottom = nil
								Captain:Say("You don't seem like the kind of kid that causes a lot of trouble, so I don't expect to see you around here anymore.", 
...
--[[                onLoad_chunk55 = function(chunk)
			    local map = chunk.map
			    if completedEvents.EBDefeat then
				map.EBBlocker:Remove()   
			    end
                end,]]
                				

				onLoad_chunk60 = function(chunk)
				     spawn(function()_p.PlayerData:completeEvent('EBDefeat') end)
		...
		onLoad_chunk56 = function(chunk)
			...
			local Tess = chunk.npcs.Tess
			local Look = chunk.map.LookAtPart.Position
			local HOOPA; spawn(function() HOOPA = _p.DataManager:request({'Model', 'Hoopa'}) end)
			local DJ; spawn(function() DJ = _p.DataManager:request({'Model','JakePortal'}) end)
			local Portal = chunk.map.Portal
			local ThePort = chunk.map.ThePortal
			local DadP = chunk.map.DadPart.Position
			local Jake = chunk.npcs.Jake
			local JakeL = chunk.map.JakeLook.Position
			local JTP = chunk.map.JakeTween.CFrame
			local JL2 = chunk.map.JakeLook2.Position
			local Player2 = chunk.map.You2.Position
			local EC1 = chunk.npcs.EclipseAdmin1
			local EC2 = chunk.npcs.EclipseAdmin2
			local Tyler = chunk.npcs.Tyler
			local IntoPortal = chunk.map.IntoPortal
			local DadW = chunk.map.DadWalk
			local MomW = chunk.map.MomWalk
			local AvB = chunk.map.AvoidBox.Position
			local Unblock = chunk.map.Unblocker
			local block = chunk.map.TheTrigger
			local db = false
			if not completedEvents.EBDefeat then return end
			if not completedEvents.DefeatHoopa then
			block.Touched:connect(function(p)
				if db or not p or not p.Parent or players:GetPlayerFromCharacter(p.Parent) ~= _p.player then return end
				db = true
					local cam = workspace.CurrentCamera
					...
					local camCFrame1 = chunk.map.camCFrame1.CFrame
					local camCFrame2 = chunk.map.camCFrame2.CFrame
					local camCFrame3 = chunk.map.camCFrame3.CFrame
					local camCFrame4 = chunk.map.camCFrame4.CFrame
					local camCFrame5 = chunk.map.camCFrame5.CFrame
					local camCFrame6 = chunk.map.camCFrame6.CFrame
					local BFocus = chunk.map.BrickFocus.CFrame
					local JCam = chunk.map.JakeDeath.CFrame
					local HoopaEyes1 = CFrame.new(Vector3.new(-547.43, 744.764, 474.37),Vector3.new(-547.43, 744.764, 475.37))
					local HoopaEyes2 = CFrame.new(Vector3.new(-547.43, 744.764, 475.54),Vector3.new(-547.43, 744.964, 476.54))
					local HoopaEyes3 = CFrame.new(Vector3.new(-547.43, 744.764, 480.17),Vector3.new(-547.43, 744.564, 481.17))
					local HoopaEyes4 = CFrame.new(Vector3.new(-547.43, 744.764, 484.06),Vector3.new(-547.43, 744.964, 485.06))
					local HoopaEyes5 = CFrame.new(Vector3.new(-547.43, 744.764, 487.09),Vector3.new(-547.43, 744.564, 488.09))
				...
				spawn(function()
					...
					spawn(function()_p.Menu:disable()end)
					MasterControl.WalkEnabled = false
					MasterControl:WalkTo(Player)
					MasterControl:LookAt(Vector3.new(-549.32, 718.75, 545.393))
					_p.MusicManager:stackMusic(4647898813, 'Cutscene', .8)
					MasterControl:Stop()
					block:Remove()
					prof:Say('Hahahahahaha... You never give up, do you?',
						'You just don\'t know when to quit.',
						'I knew that about you, ever since that little incident with Linda in Gale Forest.',
						'Your parents thought they could hide that precious key from me by giving it to you.')
					prof:LookAt(DadP)
					Dad:Say('We were right to do so.',
					'We knew after we had shown you the key, your fascination with it was too serious.',
					'By sending it with ' .. _p.PlayerData.trainerName .. ', it would be far away from your hands.')
					prof:LookAt(Player)
					prof:Say('And so it was, for quite some time.',
					'See there was no easy way to take it from you.',
					'As it turns out, it was easier to have you deliver it personally.',
					'And here we are, all gathered together, exactly as I planned.',
					'Hit it, Tyler!')
					spawn(function()
					Utilities.sound(4591549719, nil, nil, 5)
					local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame2))
					Tween(1.5, 'easeOutQuad', function(a)
						local cf = lerp(a)
						cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
				end)
					local EGui = chunk.map.ZapGui.SurfaceGui
					for i,v in pairs(EGui:GetChildren()) do
						spawn(function()
							while true do
								wait(math.random(0.01,1.4))
								v.Visible = true
								wait(math.random(0.01,0.4))
								v.Visible = false
							end
						end)
					end
				...
				prof:Say('We\'ve awakened the portal...',
				'...and she summons her missing piece')
				prof:WalkTo(Player-Vector3.new(0,0,2))
				prof:Say('Thank you very much, '.._p.PlayerData.trainerName)
					wait(.5)
					prof:WalkTo(Look)
					wait(1)
					for i,v in pairs(ThePort.MyBrick:GetChildren()) do
						v.Transparency = 0
					end
					wait(1)
					prof:WalkTo(Vector3.new(-549.32, 718.75, 545.393))
				wait(1)
					local chains = chunk.map.Chains					
				...
				wait(1.5)
				for i,v in pairs(Unblock:GetChildren()) do
					if v:IsA("Part") then
						v.Anchored = false
						wait(math.random(0.8,1.3))
					end
				end
				local E2Pos = EC2.model.HumanoidRootPart.Position
				prof:LookAt(Player)
				prof:Say('Harry, the bottle.')
				prof:LookAt(E2Pos)
				EC2:Say('Yes, sir.')
				EC2:WalkTo(prof.model.HumanoidRootPart.Position + Vector3.new(2,0,2))
				wait(0.5)
					spawn(function()
						EC2:WalkTo(E2Pos)
						EC2:LookAt(prof.model.HumanoidRootPart.Position)
					end)
					local PB = chunk.map.PrisonBottle:Clone()
					local ghost = chunk.map.GhostGui
					spawn(function()
						local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame3))
						Tween(3, 'easeOutQuad', function(a)
							local cf = lerp(a)
							cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
						end)
					end)
					...
					prof:WalkTo(Look)
					wait()
					local Idle = prof.humanoid:GetPlayingAnimationTracks()[1]
					Idle:Stop(0)
					local rs = prof.model.Torso['Right Shoulder']
					local rarm = prof.model["Right Arm"]
					local da = 3
					rs.DesiredAngle = da
					PB.Parent = chunk.map
					repeat
						stepped:wait()
						PB.CFrame = rarm.CFrame * CFrame.new(0, -1, 0)
					until math.abs(rs.CurrentAngle-da) < .05
					da = 1.3
					rs.DesiredAngle = da
					repeat
...
--					rs.MaxVelocity = .1
					Idle:Play()
					local cf = PB.CFrame
					local vi = Vector3.new(0, 1, -3).unit*10
					local a = Vector3.new(0, 0, -3)
					local planeZ = 520
					local st = tick()
					repeat
						stepped:wait()
						local t = tick()-st
						PB.CFrame = cf + vi*t + .5*a*t^2
					until PB.Position.Z < planeZ
					PB.CFrame = PB.CFrame + Vector3.new(5, 0, 0)
					PB.Anchored = true
					spawn(function()
						wait(1)
						PB.Anchored = false
					end)
					local pos = Vector3.new(PB.Position.X, PB.Position.Y, PB.Position.Z)
					print(pos.X,pos.Y,pos.Z)
					ghost.g.Enabled = true
					ghost.Position = pos
					local lerp = select(2, Utilities.lerpCFrame(ghost.CFrame, ghost.CFrame-Vector3.new(0,-10,50)))
					Tween(1, 'easeInOutSine', function(a)
						local cf = lerp(a)
						ghost.CFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
					ghost.g.Enabled = false
				Utilities.FadeOut(2,Color3.new(1,1,1))
				cam.CFrame = CFrame.new(HoopaEyes1.Position,HoopaEyes1.Position+Vector3.new(0,0,1))
					wait()
					prof:WalkTo(JakeL)
				Utilities.FadeIn(1.5)
					local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, HoopaEyes2))
					Tween(1, 'easeInOutSine', function(a)
						local cf = lerp(a)
						cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
					local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, HoopaEyes3))
					Tween(1, 'easeOutQuad', function(a)
						local cf = lerp(a)
						cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
					wait(0.5)
					local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, HoopaEyes4))
					Tween(1, 'easeInOutSine', function(a)
						local cf = lerp(a)
						cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
					local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, HoopaEyes5))
					Tween(1, 'easeOutQuad', function(a)
						local cf = lerp(a)
						cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
					Utilities.FadeOut(1)
					cam.CFrame = CFrame.new(camCFrame4.Position + Vector3.new(0,0,25),camCFrame4.Position)
					HOOPA.Parent = chunk.map
					Sprite:playCry(1, _p.DataManager:getSprite('_FRONT', 'Hoopa-Unbound').cry,1)
					Utilities.FadeIn(1)
					shake(3,1)
					wait(.35)
					spawn(function()
				...
				prof:Say('I just love it when everything goes according to plan!')
				Dad:Say('You\'re making a huge mistake.',
				'You don\'t know what kind of power you\'re messing with.')
				prof:Say('Ah, that\'s where you\'re wrong.',
				'I have a pretty good idea of the power I\'m messing with.')
				prof:LookAt(Look)
					cam.CFrame = CFrame.new(camCFrame4.Position + Vector3.new(0,0,25),camCFrame4.Position)
					spawn(function()
						chat.bottom = true
						chat:say('It\'s the power to travel to other worlds.',
						'The power to become the creator of my own reality.',
							'The power to control right and wrong.')
						chat.bottom = nil
					end)
					local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame4))
					Tween(7.5, 'easeOutQuad', function(a)
						local cf = lerp(a)
						cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
					local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame1))
					Tween(2, 'easeOutQuad', function(a)
						local cf = lerp(a)
						cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
				prof:LookAt(Player)
				chat:say(_p.PlayerData.trainerName.. ', I have no further need of you.',
				'I suspect that I will have to defeat you in battle to get rid of you for good.',
				...
				local win = _p.Battle:doTrainerBattle {
					...
					musicId = 506415622,
					vs = {name = 'Boss Cypress', id = 506375182, hue = 1/12}
				}
				if not win then
					_p.RunningShoes:enable()
					MasterControl.WalkEnabled = true
				...
				prof:Say('Enough nonsense!')
				prof:LookAt(Look)
				prof:Say('Hoopa!',
				'It is I who has awoken you this day!',
				'Grant me my wish, that I may discover a pure world in need of ruling!')
				Utilities.FadeOut(.2,Color3.new(255,255,255))
				Sprite:playCry(1, _p.DataManager:getSprite('_FRONT', 'Hoopa-Unbound').cry, 1)
				local sg = create 'SurfaceGui' {
					CanvasSize = Vector2.new(252, 252),
					Face = Enum.NormalId.Front,
				...
				anim.spriteLabel.Parent = sg
					...
					Utilities.FadeIn(.2)
					wait(1)
				prof:Say('Isn\'t it glorius?',
				'You will all soon see that I\'m not as mad as you believe.',
				'In fact, I am the most clever of any of you.',
				'I am the only soul wise enough to devise such a plan, and brave enough to carry it out.',
				'The almighty Hoopa, a Pokemon so talented and powerful...',
				'Ancient Rorians were so full of fear, they decided to imprison it here.')
				Utilities.FadeOut(.5)
				chat:say('And here it stayed, waiting for me to come along one day an...',
				'WHAT ARE YOU DOING?!')
				prof:Remove()
				Jake:WalkTo(Vector3.new(-552.982, 716.245, 545.026))
				Utilities.FadeIn(.5)
				spawn(function()
					local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame5))
					Tween(1.5, 'easeOutQuad', function(a)
						local cf = lerp(a)
						cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)-- no roll
					end)
					...
					Jake:WalkTo(JakeL)
					spawn(function()Utilities.exclaim(EC2.model.Head)end)
					spawn(function()Utilities.exclaim(EC1.model.Head)end)
				EC1:LookAt(JakeL)
				EC2:LookAt(JakeL)
				Jake:LookAt(EC2.model.HumanoidRootPart.Position)
				Jake:Say('Am I the only one that\'s tired of that guy\'s monologuing?',
					'I\'ve been holding that in for a long time...'
				)
				EC2:Say('Jake, what is the meaning of this?')
				Jake:Say('It\'s the end of the line for Team Eclipse.',
					'Nobody comes between me and my friends.',
					'Cyprus made a huge mistake thinking he could take advantage of us like that.'
				)
				EC2:Say('You\'re making a huge mistake if you think you\'ve stopped Team Eclipse.',
					'Cyprus is on his way to our new world as we speak!',
					'This is only the beginning!'
				)
				Jake:Say('You\'d better join him, or you\'re next!')
					EC2:Say('Say no more!')
					spawn(function()
						local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame1))
						Tween(1.5, 'easeOutQuad', function(a)
				...
				Jake:LookAt(IntoPortal.Position)
				spawn(function() EC2:WalkTo(IntoPortal.Position) EC2:Remove() end)
				spawn(function() EC1:WalkTo(IntoPortal.Position) EC1:Remove() end)
				spawn(function() Tyler:WalkTo(IntoPortal.Position) Tyler:Remove() end)
					wait(1)
					spawn(function()
						local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame6))
						Tween(1.5, 'easeOutQuad', function(a)
							local cf = lerp(a)
							cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
						end)
					end)
					spawn(function()MasterControl:WalkTo(Player2)end)
					Jake:WalkTo(JL2)
					spawn(function()MasterControl:LookAt(JL2)end)
					Jake:LookAt(Player2)
				Jake:Say('The portal will continue to grow out of control until Hoopa is defeated!',
					'I will stay here and fight.',
					'The rest of you need to leave now.'
				)
				Jake:LookAt(Tess.model.HumanoidRootPart.Position)
				Tess:Say('Jake, we have fought for so long to rescue you!',
					'I am not leaving your side now.'
				)
				Jake:Say('The fight is not over yet.',
					'I need you to get to safety so that I can finish this!'
				)
				Jake:LookAt(Player2)
				Jake:Say('The ancient Rorians bound Hoopa because it Removeed their homes and way of life.',
					'If we arent careful, the same will happen to us.',
					'Cyprus planned to leave this world to be Removeed by Hoopa.',
					'Hoopa must be stopped before it\'s too late.'
				)
				Jake:LookAt(Tess.model.HumanoidRootPart.Position)
				Jake:Say('Please go, Tess.',
				...
				spawn(function()
					Jake:LookAt(IntoPortal.Position)
					Tess:LookAt(IntoPortal.Position)
				end)

				delay(1.2, function() shake(1.5) end)
				delay(2, function() shake(1) end)
				delay(2.9, function() shake(1) end)
				delay(3.6, function() shake(1) end)
				delay(4.9, function() shake(.5) end)
				Tess:Say('The portal...',
					'Its getting too unstable!',
					'This can\'t be good!'
				)
				Jake:Say('This is exactly what I feared would happen.',
					'The portal is only going to keep getting worse.',
					'You all Need to run now!'
				)
				Dad:Say('Let\'s Go, Son!')
				spawn(function()Utilities.exclaim(Dad.model.Head) end)
				spawn(function()Utilities.exclaim(Mom.model.Head) end)
				spawn(function()Dad:LookAt(Tess.model.HumanoidRootPart.Position) end)
				spawn(function()Mom:LookAt(Tess.model.HumanoidRootPart.Position) end)
				MasterControl:LookAt(Tess.model.HumanoidRootPart.Position)
				Tess:Say('Jake, you have to stop trying to do this alone.',
					'Friends like us are always there for each other.',
					'We can finish this together if we work as a team.'
				)
					local Box = chunk.map.WoodCrate.Center
					local BoxPos1 = chunk.map.BoxPos1.CFrame
					local BoxPos2 = chunk.map.BoxPos2.CFrame
					local BoxPos3 = chunk.map.BoxPos3.CFrame
					local CamPos = chunk.map.camCFrame7.CFrame
					local CamPos2 = chunk.map.camCFrame8.CFrame
					repeat wait() until DJ ~= nil
					local hrp = DJ.TweenPart
						local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, CamPos))
						Tween(1.5, 'easeOutQuad', function(a)
							local cf = lerp(a)
							cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
						end)
					local lerp = select(2, Utilities.lerpCFrame(Box.CFrame, BoxPos1))
					Tween(2, 'easeOutQuad', function(a)
						local cf = lerp(a)
						Box.CFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
					wait(.3)
					local lerp = select(2, Utilities.lerpCFrame(Box.CFrame, BoxPos2))
					Tween(1.2, 'easeOutQuad', function(a)
						local cf = lerp(a)
						Box.CFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
					cam.CoordinateFrame = CamPos2
					wait(.5)
					Jake:LookAt(Player2)
					MasterControl:WalkTo(AvB)
					Jake:Say('WATCH OUT!')
					spawn(function()
						local lerp = select(2, Utilities.lerpCFrame(Box.CFrame, BoxPos3))
						Tween(2, 'easeOutQuad', function(a)
							local cf = lerp(a)
							Box.CFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
						end)
					end)
					local JRoot = Jake.model.HumanoidRootPart
					local Dive = Jake.model.Humanoid:LoadAnimation(create 'Animation'{ AnimationId = 'rbxassetid://'.._p.animationId.JakeDive })
					local Fall = Tess.model.Humanoid:LoadAnimation(create 'Animation'{ AnimationId = 'rbxassetid://'.._p.animationId.TessFall })
					Jake:LookAt(Tess.model.Head.Position)
					spawn(function()Tess:LookAt(Jake.model.Head.Position)end)
					spawn(function()Dive:Play(1.5)end)
					wait(1)
					Fall:Play(1.5)
					wait(0.7)
					Box.Parent:Remove()
					...
					Jake:Remove()
					spawn(function()
						local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, JCam + Vector3.new(0,0,-5)))
						Tween(6, 'easeOutQuad', function(a)
							local cf = lerp(a)
							cam.CoordinateFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
						end)
					end)
					local lerpa = select(2, Utilities.lerpCFrame(hrp.CFrame, JTP))
					Tween(6, 'easeInQuad', function(a)
						local cf = lerpa(a)
						hrp.CFrame = cf
					end)
					DJ:Remove()
					...
					wait(1)
					Tess:Say('NOOOOOOOOOOO!!')
					Sprite:playCry(1, _p.DataManager:getSprite('_FRONT', 'Hoopa-Unbound').cry, 1)
					for i,v in pairs(ThePort:GetDescendants()) do
						if v:IsA("Part") or v:IsA("UnionOperation") then
							v.Anchored = false
						end
					end
				anim.spriteLabel.Parent = nil
				Dad:Say(_p.PlayerData.trainerName..' It\'s up to you!',
					'Stop Hoopa now, before it creates another portal'
				)
				Utilities.FadeOut(0.5)
				pcall(function()HOOPA:Remove()end)
				local HoopaEncounter = chunk.data.regions['Aborille Outpost'].Hoops
				spawn(function()Utilities.FadeIn(1) end)
					local win = _p.Battle:doWildBattle(HoopaEncounter,{BattleSceneType = 'Tomb',LeaveCameraScriptable = true,musicId = 594478911,})
				if not win then
					MasterControl.WalkEnabled = true
					chat:enable()
					_p.Menu:enable()
					...
					MasterControl.WalkEnabled = false
					_p.Menu:disable()
					spawn(function()MasterControl:WalkTo(Player2) MasterControl:LookAt(DadP)end)
					spawn(function()
						local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame6))
						Tween(1.5, 'easeOutQuad', function(a)
							local cf = lerp(a)
					...
					wait(1.5)
				Tess:Say('I can\'t believe what just happened.',
					'Jake was so brave.',
					'He didn\'t deserve what just happened to him...',
					'He is a hero.'
					)
					local MyBrickMain = ThePort.MyBrick.Main
					local BrickFloatPos = chunk.map.BrickCF1.CFrame
					MyBrickMain.Anchored = false
					MyBrickMain.CFrame = BrickFloatPos
					MyBrickMain.Anchored = true
					local FloatTo = CFrame.new(BrickFloatPos.p + Vector3.new(0,0,2))
					delay(2, function()
						local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame6))
						Tween(1.5, 'easeOutQuad', function(a)
							local cf = lerp(a)
					...
					MasterControl:WalkTo(BrickFloatPos.p + Vector3.new(0,0,4))
					MasterControl:LookAt(BrickFloatPos.p)
					local lerp = select(2, Utilities.lerpCFrame(MyBrickMain.CFrame, FloatTo))
					Tween(1.5, 'easeOutQuad', function(a)
						local cf = lerp(a)
						MyBrickMain.CFrame = CFrame.new(cf.p, cf.p+cf.lookVector)
					end)
					for i,v in pairs(MyBrickMain.Parent:GetChildren()) do
						v.Anchored = true
					end
					chat.bottom = true
					onObtainKeyItemSound()
					spawn(function()MyBrickMain.Parent:Remove()end)
					chat:say('Bronze Brick obtained!',
						_p.PlayerData.trainerName..' put the Bronze Brick in the bag!'
					)
					chat.bottom = nil
					spawn(function()
						local lerp = select(2, Utilities.lerpCFrame(cam.CoordinateFrame, camCFrame6))
					...
					MasterControl:WalkTo(Player2)
					MasterControl:LookAt(DadP)
					Dad:LookAt(Player2)
					Dad:Say('It seems that the bronze brick trusts you to be it\'s protector!',
						'I\'m proud of you for becoming such an incredible Pokemon Trainer!',
						'You came quite a long way to save us and Jake.',
						'This could have gone quite differently had you not been here.'
					)
					Mom:Say('Yes that was quite surprising!',
						'Anyway, I think we should all go home and rest in order to recover from this traumatic experience.'
					)
					Tess:Say('Im going to catch Gerald up on all this.',
						'I\'ll see if he knows any way we can save Jake.',
						'Unfortunately I wont be able to see you through the Roria league, but I know that you\'ll do it.'
					)
					Dad:Say('We Hope to see you again, Tess')
					Utilities.FadeOut(1, Color3.new(255,255,255))
					spawn(function()_p.PlayerData:completeEvent('DefeatHoopa') end)
					Utilities.TeleportToSpawnBox()
					chunk:Remove()
					-- Too make this original we did a change chunks then can defeat Gym 8
					...
					Utilities.FadeIn(1)
					spawn(function() _p.Menu:enable() end)
					spawn(function() MasterControl.WalkEnabled = true end)
				end)
			else
				print('Completed.. Finally - Shadows')
				Dad:Remove()
				Mom:Remove()
				EC1:Remove()
				EC2:Remove()
				...
				onLoad_chunk62 = function(chunk) -- ANTHIAN: BATTLE
					local map = chunk.map
					--if not Utilities.isTouchDevice() and not _p.Menu.options.reduceGraphics then
					--Battling On the Grounds # Logic
					local Slr = chunk.npcs.Seller
					interact[Slr.model] = function()
						spawn(function() _p.Menu:disable() end)
						chat:say(Slr, 'Im selling items, Heres my Catalog.')
						_p.Menu.battleShop:open()
						chat:say(Slr, 'If you need anything else you know where to look.')
						spawn(function() _p.Menu:enable() end)
					end
					local cmptv = chunk.npcs.BeachTrainer
					interact[cmptv.model] = function()
						if not cmptv:Say('Hello There!',
							'I see you\'ve come to Challenge me.',
							'Be warned Trainer, My Pokemon are Trained to Max Level!',
							'[y/n]Are you Ready to Battle?') then
							cmptv:Say('If you change your mind I\'ll be waiting.')
							return
						end
						cmptv:Say('Alright, let\'s do this!')
			--[[cmptv:Say('This park is usually filled with competitors...',
					'I guess they are all taking a break.',
					'Since theres nobody battling...',
					'We should Battle Right?',
					'I hope your ready!')]]
						local win = _p.Battle:doTrainerBattle {
							musicId = 2049088345,
							musicVolume = .6,
							PreventMoveAfter = true,
							LeaveCameraScriptable = true,
							trainerModel = cmptv.model,
							num = 183
						}
						if win then
							cmptv:Say('I did not Expect this!',
								'I always love Pokemon Battles.',
								'I\'ll be looking forward to a Rematch soon.',
								'Maybe Next time i will win.',
								'Until then Fellow Trainer!')
							Utilities.lookBackAtMe()
							MasterControl.WalkEnabled = true
							_p.Menu:enable()
							chat:enable()
				...
				onLoad_chunk64 = function(chunk)
					local Ldy = chunk.npcs.Lady
					interact[Ldy.model] = function()
						if not Ldy:Say('Hello!',
							'[y/n]Are you ready to go back?') then
							Ldy:Say('Oh Alright, Tell me when you feel like going back.')
							return
						end
						Ldy:Say('Alright, let\'s Go!')
						Utilities.FadeOut(.5)
						local startTick = tick()
						Utilities.TeleportToSpawnBox()
						chunk:Remove()
						...
						workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
						Utilities.Teleport(CFrame.new(-369.596, 34.92, -96.469))
						local elapsed = tick()-startTick
						if elapsed < .5 then
							wait(.5-elapsed)
						end
			...
			local map = chunk.map
			if not map:FindFirstChild('Mew') then
				map.ComputerInteract:Remove()
			end	
		end,	
