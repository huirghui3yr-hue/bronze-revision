...
--	local event = _p.Events['modifyBattle_'.._p.DataManager.currentChunk.id] -- OVH  move to server
...
--	end
	...
	if data.vs then
		...
		local ballImage = Utilities.Create 'ImageLabel' {
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://'..(data.IconId or (data.trainer and data.trainer.IconId) or 6222192306),--IMAGE NEED TO BE REPLACED OR REUPLOADED
...
function BattleClient:blackedOut(wild, moneyLost)
	...
	if chunk.id == 'chunk1' then
		...
		stepped:wait()
		_p.MusicManager:prepareToStack(0)
		_p.MusicManager:stackMusic(288895151, 'RoomMusic')--POKECENTER MUSIC NEED TO BE REPLACED OR REUPLOADED ASAP fsdgfjhnf
		Utilities.FadeIn(1)
		...
		for i, p in pairs(pokeballs) do
			...
			Utilities.sound(301970857, 1, .1, 2) --POKEMON BEING PLACED ON HEALING MACHINE NEED TO BE REPLACED OR REUPLOADED ASAP ttr
		...
		Utilities.sound(300394295, nil, .5, 10)--POKEMON HEALED NEED TO BE REPLACED OR REUPLOADED ASAP sdfbgnhmn
...
function BattleClient:doPVPBattle(data)
	...
	_p.MusicManager:stackMusic(454019829, 'TeamPreview', .3)
...
--			battle.opponentPartyPreview = 'error'
	...
	local ballImage = Utilities.Create 'ImageLabel' {
		BackgroundTransparency = 1.0,
		Image = 'rbxassetid://5639630646',--NEED TO BE REPLACED/REUPLOADED
		SizeConstraint = Enum.SizeConstraint.RelativeYY,
		Size = UDim2.new(0.3, 0, 0.3, 0),
		Position = UDim2.new(0.08, 0, 0.6, 0),
		ZIndex = 4, Parent = Utilities.Create 'Frame' {
...
--	print('MY TEAM POSITION:', data.myTeamN)
	...
	data.ignoreNicknamesAt['22'] = true
	if not data.musicId then
		data.musicId = 314907282--NEED TO BE REPLACED/REUPLOADED ASAP ITS PVP MUSIC zry5htjg
	end
	...
	-- 2v2 TEAM PREVIEW
	_p.MusicManager:stackMusic(454019829, 'TeamPreview', .3) --REUPLOAD/REPLACE ASAP TEAM PREVIEW PVP SCREEN
	teamPreview:prepare(battle)
...
--		end
	...
	local ballImage = Utilities.Create 'ImageLabel' {
		BackgroundTransparency = 1.0,
		Image = 'rbxassetid://5639630646',--IMAGE NEEDS TO BE REUPLOADED OR REMADE
		SizeConstraint = Enum.SizeConstraint.RelativeYY,
		Size = UDim2.new(0.3, 0, 0.3, 0),
		Position = UDim2.new(0.08, 0, 0.6, 0),
		ZIndex = 4, Parent = Utilities.Create 'Frame' {
...
function BattleClient:setupScene()
	...
	if self.kind ~= 'pvp' and self.kind ~= '2v2' then
		local musicId = self.musicId or {608775689, 608776631} -- old: {424133853, 424134350}, really old: 282237556 --battle music left to right nwpb, nwpbl, wpb, wpbl and wild pokemon battle
...
--				print(s and 'new weld system success!!!' or ('new weld system failed: '..e))
		...
		root.CFrame = cframe
		return {Root = root, Humanoid = h, BodyGyro = bg, BodyPosition = bp, ThrowAnimation = throw, Model = m, Scale = scale, CFrame = cframe}
	end
