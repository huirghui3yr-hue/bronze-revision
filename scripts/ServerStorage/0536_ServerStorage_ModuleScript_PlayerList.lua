		...
		if leaderstats then
			for i, stat in ipairs(GameStats) do
				...
				if not statFrame then
					...
					if statObject then
						initializeStatText(stat, statObject, entry, statFrame, i, isTopStat)
					end
...
local function onTeamRemoved(removedTeam)
	...
	if #TeamEntries == 0 then
		if NeutralTeam then
			NeutralTeam.Frame:Remove()
			NeutralTeam.Team:Remove()
			NeutralTeam = nil
			IsShowingNeutralFrame = false
		end
	end
