...
--]]
local ITEM_SHEET_ID = 5222852432
...
function bag:getItemIcon(item)
	...
	local zMoveTable = {
        [669] = 4871838107, -- Grass
		[670] = 4855881937, -- Fire
		[671] = 4871366610, -- Water
        [672] = 4871838430, -- Ice
        [673] = 4871837488, -- Bug
		[674] = 4871837890, -- Dragon
	}
	local function isZMove() local s, e = pcall(function() return zMoveTable[icon] end) if s then return zMoveTable[icon] end end
	local s = 0.9
	return create 'ImageLabel' {
		Name = 'ItemIcon',
		BackgroundTransparency = 1.0,
		Image = 'rbxassetid://'..(not isZMove() and ITEM_SHEET_ID or tostring(zMoveTable[icon])),
		ImageRectSize = not isZMove() and Vector2.new(32, 32) or Vector2.new(0, 0),
		ImageRectOffset = not isZMove() and Vector2.new(32*((icon-1)%26), 32*math.floor((icon-1)/26)) or Vector2.new(0, 0),
...
function bag:updatePartyList(pouch, specificPokemonIndex)--::updatePartyList
	local tm
	if type(pouch) == 'table' then
		tm, pouch = pouch, nil
	end
	fastSpawn(function()
...
function bag:useSelectedItem()--::useSelectedItem
	...
	local pokemon, index, bg, txtRF, cancel
