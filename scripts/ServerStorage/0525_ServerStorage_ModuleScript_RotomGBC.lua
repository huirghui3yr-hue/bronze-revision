...
local function write(text, parent, px, py, timedFill, rightJustify)
	...
	local frame = create 'Frame' {
		...
		ZIndex = parent.ZIndex, Parent = parent,
	}
...
local function makeBorderedFrame(sx, sy, px, py, zindex)
	if sx < 3 or sy < 3 then error('bad frame size', 2) end
	zindex = zindex or 1
	local frame = create 'Frame' {
		BorderSizePixel = 0,
		BackgroundColor3 = BACKGROUND_COLOR,
	...
	create 'ImageLabel' {
		...
		Image = SHEET_ID,
		ImageRectSize = Vector2.new(8*4, 8*4),
		ImageRectOffset = Vector2.new(2*8*4, 0),
		Size = UDim2.new(1/sx, 0, 1/sy, 0),
		Position = UDim2.new(1-1/sx, 0, 0.0, 0),
		ZIndex = zindex, Parent = frame,
	}
	-- left
	create 'Frame' {
	...
	create 'ImageLabel' {
		...
		Image = SHEET_ID,
		ImageRectSize = Vector2.new(8*4, 8*4),
		ImageRectOffset = Vector2.new(0, 2*8*4),
		Size = UDim2.new(1/sx, 0, 1/sy, 0),
		Position = UDim2.new(0.0, 0, 1-1/sy, 0),
		ZIndex = zindex, Parent = frame,
	}
	--bottom
	create 'Frame' {
	...
	create 'ImageLabel' {
		...
		Image = SHEET_ID,
		ImageRectSize = Vector2.new(8*4, 8*4),
		ImageRectOffset = Vector2.new(2*8*4, 2*8*4),
		Size = UDim2.new(1/sx, 0, 1/sy, 0),
...
create 'ImageLabel' { -- top-left
	BackgroundTransparency = 1.0,
	Image = 'rbxassetid://347416341',
	Size = UDim2.new(192/352, 0, 192/352, 0),
...
create 'ImageLabel' { -- top-right
	...
	Size = UDim2.new(160/352, 0, 192/352, 0),
	Position = UDim2.new(192/352, 0, 0.0, 0),
	ZIndex = 4, Parent = worldmap,
}
create 'ImageLabel' { -- bottom-left
	BackgroundTransparency = 1.0,
	Image = 'rbxassetid://347416317',
	Size = UDim2.new(192/352, 0, 160/352, 0),
	Position = UDim2.new(0.0, 0, 192/352, 0),
	ZIndex = 4, Parent = worldmap,
}
create 'ImageLabel' { -- bottom-right
	BackgroundTransparency = 1.0,
	Image = 'rbxassetid://347416293',
	Size = UDim2.new(160/352, 0, 160/352, 0),
	Position = UDim2.new(192/352, 0, 192/352, 0),
	ZIndex = 4, Parent = worldmap,
}
local cave = create 'ImageLabel' {
	BackgroundTransparency = 1.0,
	Image = 'rbxassetid://347416268',
...
do
	local function onRunClicked()
		...
		setZIndex(chatBox, 6)
		say(true, 'Ca[n\'t] escape!')
		setZIndex(chatBox, 3)
		interaction:setContext('BattleMainMenu')
	end
	local fight = interaction:registerSelectable('BattleMainMenu', 10, 14, 5, 1, false, nil, function() mainBattleMenu.Parent = nil; moveInfo.Parent = screen; movesMenu.Parent = screen; interaction:setContext('BattleMovesMenu') end)
	local pkmn  = interaction:registerSelectable('BattleMainMenu', 16, 14, 2, 1, false, nil, function() end)
	local item  = interaction:registerSelectable('BattleMainMenu', 10, 16, 4, 1, false, nil, function() mainBattleMenu.Parent = nil; itemMenu.Parent = screen; interaction:setContext('ItemMenu') end)
...
do
	local fossil = interaction:registerSelectable('ItemMenu', 6, 4, 12, 1, false, nil, function()
		...
		say(false, 'OAK: RED!', 'This is[n\'t] the', 'time to use that!')
		ea:Remove()
		setZIndex(chatBox, 3)
		interaction:setContext('ItemMenu')
	end)
	local cancel = interaction:registerSelectable('ItemMenu', 6, 6, 12, 1, false, nil, function() itemMenu.Parent = nil; mainBattleMenu.Parent = screen; interaction:setContext('BattleMainMenu') end)
	fossil.down = cancel
	cancel.up   = fossil
end
...
battle = function()
	...
	_p.MusicManager:stackMusic(358961364, 'RotomBattle', .5)
...
--		battle()
	...
	_p.MusicManager:stackMusic(333249262, 'LavenderTown', .3)
end
