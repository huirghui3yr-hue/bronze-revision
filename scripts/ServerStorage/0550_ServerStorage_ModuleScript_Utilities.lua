...
do
	...
	function util.setupRemoveWatch()
		...
		for _, class in pairs(classes) do
			local oldRemove = class.class.Remove
			if not oldRemove then
--				warn(class.name .. ' does not have a Remove() function')
				class.lacksRemove = true
			end
			class.class.Remove = function(obj, ...)
				...
				if oldRemove then oldRemove(obj, ...) end
...
if player and not script.Parent:IsA('ServerStorage') then
	...
	util.backGui.IgnoreGuiInset = true
...
function util.loopSound(id, volume, loopId)
	...
	if id == 317469005 then volume = .7 end -- eclipse battle music need to be replaced/reuploaded TEABM3eqrfd
	local sound = util.Create 'Sound' {
		SoundId = type(id)=='number' and ('rbxassetid://'..id) or id,
		Volume = volume or .5,
		Looped = loopId==nil,
