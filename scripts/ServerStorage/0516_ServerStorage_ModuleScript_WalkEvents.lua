...
--		lastEggStepTick = tick()--
	...
	local grass
	if inGrass or (regionData.GrassNotRequired and (not regionData.NoGrassIndoors or not _p.DataManager.currentChunk.indoors)) then
		grass = regionData.Grass
	elseif inMiscGrass then
		grass = regionData.MiscEncounter
	end
	if not grass or _p.Battle.currentBattle or not MasterControl.WalkEnabled then return end
...
--				FormFactor = Enum.FormFactor.Custom,
				...
				create 'Decal' {
					...
					Texture = 'rbxassetid://329666477',--330318672
					Shiny = 0,
					Specular = 0,
				}
			}
