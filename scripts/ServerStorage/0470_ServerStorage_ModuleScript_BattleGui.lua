...
function BattleGui:animAbility(poke, abilityName)
	...
	local gui = create 'ImageLabel' {
		BackgroundTransparency = 1.0,
		Image = 'rbxassetid://'..({5217870014, 5217868182})[n],
...
function BattleGui:animBoost(poke, good)
	...
	Utilities.sound(good and 301970798 or 301970736, .3, nil, 5)--301970798 is stats being raised and 301970736 is stats decreasing, replace before they delete  asap
	spawn(function()
		local angles = {}
...
function BattleGui:animHit(target, source, type, soundid, effectiveness, suppressParticles)
	...
	local smack = create 'ImageLabel' {
		Name = 'SmackAnim',
		BackgroundTransparency = 1.0,
		Image = 'rbxassetid://5210269907',
		ImageColor3 = color,
		Parent = Utilities.gui,
	}
	Utilities.sound(soundid or 6547048561, .75, effectiveness == 1 and .5 or .6, 5)--normal damage normaldamage replace?
...
function BattleGui:animMove(battle, pokemon, move, targets)
	...
	for _, a in pairs(battle.actionQueue) do
		...
		elseif arg1 == '-supereffective' then
			...
			if target then
				effectives[target] = 2
				soundids[target] = 6547057340--super effective replace?
			end
		elseif arg1 == '-resisted' then
			local target = battle:getPokemon(args[2])
			if target then
				effectives[target] = .5
				soundids[target] = 6547129063--not very effective, replace?
			end
		end
	end
	if moveAnims[move.id] then
		...
		if continue == 'sound' then -- TODO: this could be better; for now, ONLY RETURN THIS if the move has a single target
			...
			Utilities.sound(s or 6547048561, .75, e == 1 and .5 or .6, 5)--normal damage replace maybe
			return
		end
...
do -- original code
	...
	local msgBox = create 'ImageLabel' {
		Name = 'BattleMsg',
		BackgroundTransparency = 1.0,
		Image = 'rbxassetid://6222466249',--NEED TO BE REPLACED THERES NO IMAGE
		ImageTransparency = 1.0,
		Parent = Utilities.frontGui,
	...
	if gui.main then
		...
		local function b(name, labelColor, textColor)
			local label = create 'ImageLabel' { -- 522 x 130
				Name = name,
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://6298426446',--REPLACE THERES NOTHING HERE (GREY SCALE BUTTON IDK WHAT THAT IS)
		...
		local cancel = create 'ImageLabel' { -- 522 x 130
			Name = 'Cancel',
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://6298426446',--this too greyscalebutton
	...
	if not moves then
		...
		for i = 1, 4 do
			moves[i] = create 'ImageLabel' { -- 522 x 130
				Name = 'Move'..i,
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://6298426446',--another grey scale button
				Size = UDim2.new(1.0, 0, 1.0, 0),
				Parent = container,
		...
		-- #MEGA
		mega = _p.AnimatedSprite:new{sheets={{id=5120126527,rows=10}},nFrames=20,fWidth=393,fHeight=99,framesPerRow=2,button=true}
...
--		BackgroundColor3 = Color3.new(.3, .3, .3),
		...
		create 'ImageLabel' {
			Name = 'OwnedIcon',
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://6222192306',--no clue what this is, ownedicon?
			SizeConstraint = Enum.SizeConstraint.RelativeYY,
...
--		BackgroundColor3 = Color3.new(.3, .3, .3),
		...
		create 'ImageLabel' {
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://5210243574',--5210245467',
			ImageColor3 = Color3.new(.5, .5, .5),
			Size = UDim2.new(1.0, 0, 1.0, 0),
			Position = UDim2.new(0.0, 0, 0.05, 0),
		},
...
function BattleGui:promptReplaceMove(movesData, alreadyFaded)
	...
	local function guiForMove(move, onClick)
		...
		local panel = roundedFrame:new {
			...
			create 'ImageLabel' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://'..({Status=6223782292,Special=6223783835,Physical=6223776992})[move.category],--ALL 3 NEED TO BE REPLACED
...
--	local sd2 = _p.DataManager:getSprite((shiny and '_SHINY' or '')..'_FRONT', p2, p.gender=='F')
	...
	Utilities.sound(287784334, nil, nil, 5)--POKEMON EVOLVE START NEED TO BE REPLACED
	local sound
	delay(1, function()
		if canceled then return end
		sound = Utilities.loopSound(287785336)--POKEMON EVOLVING REPLACE
	end)
	...
	spawn(function()
		...
		for i = 1, n do
			...
			local image = Utilities.Create 'ImageLabel' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://6523524045',--5210256242
...
--		topGui:Remove()
	...
	Utilities.sound(278827181, nil, .5, 10)--POKEMON EVOLVED END REPLACE
