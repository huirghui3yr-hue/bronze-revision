	...
	function rtd:playerSearch()
		...
		if not gui then
			...
			local cancel = _p.RoundedFrame:new {
				...
				Position = UDim2.new(.775, 0, .02, 0),--isPhone and UDim2.new(33/32, 0, 7/16, 0) or UDim2.new(3/4, 0, 1/16, 0), <-- does that even work?? might be useful for mobile
				ZIndex = 3,
				Parent = gui.gui,
...
--			header.BackgroundTransparency = 1.0
			...
			local joinButton = create 'TextButton' {
				...
				MouseButton1Click = function()
					...
					self:teleport(searchPlaceId, searchJobId)
				end,
			}
			write 'Join' {
				Frame = create 'Frame' {
		...
		Utilities.fastSpawn(function()
			local spinner = create 'ImageLabel' {
				BackgroundTransparency = 1.0,
					Image = 'rbxassetid://7214222605', -- 5685510578,7214222605
...
function rtd:enable()
	...
	if not gui then
		gui = create 'ImageLabel' { -- 184 x 389
			BackgroundTransparency = 1.0,
				Image = 'rbxassetid://6298178827', -- 313053191 -- 311460172
