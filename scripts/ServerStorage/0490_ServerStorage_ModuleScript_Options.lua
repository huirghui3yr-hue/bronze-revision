...
local options = {
	...
	dialogSpeed = 35, -- characters per second; Slow=18, Normal=35, Fast=70
}
local gui, bg, close, unstuckButton, unstuckTimerContainer

local unstuckCooldown = 5 * 60
...
function options:open()
	...
	if not gui then
		...
		local speedLabels = {'Slow', 'Normal', 'Fast'}
		local speedValues = {18, 35, 70}
		local speedButtons = {}
		local function getCurrentSpeedIndex()
			for i, v in ipairs(speedValues) do
				if options.dialogSpeed == v then return i end
			end
			return 2
		end
		local function updateSpeedButtons(selected)
			for i, btn in ipairs(speedButtons) do
				btn.BackgroundColor3 = (i == selected) and color(80, 160, 80) or color(80, 80, 80)
			end
		end
		for i, label in ipairs(speedLabels) do
			local btn = _p.RoundedFrame:new {
				Button = true,
				BackgroundColor3 = color(80, 80, 80),
				Size = UDim2.new(0.2, 0, 0.08, 0),
				Position = UDim2.new(0.27 + (i-1)*0.22, 0, 0.375, 0),
				ZIndex = 3, Parent = gui,
				MouseButton1Click = function()
					options.dialogSpeed = speedValues[i]
					updateSpeedButtons(i)
				end,
			}
			write(label) {
				Frame = create 'Frame' {
					...
					ZIndex = 4, Parent = btn.gui,
				}, Scaled = true,
			}
			speedButtons[i] = btn
		end
		updateSpeedButtons(getCurrentSpeedIndex())
		...
		unstuckTimerContainer = create 'Frame' {
			...
			-- Bag
			--gotta test to fix it because its not working for some reason
			--[[		
		...
		write 'Wearable Bag' {
			...
			--]]
			--its still not working e.e
			--all references to this bag are in Misc line 309 and WearableBag, ill fix when i have the time
			--]]
