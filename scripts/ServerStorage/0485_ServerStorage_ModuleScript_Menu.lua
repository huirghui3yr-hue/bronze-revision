...
function Menu:enable()
	...
	if not container then
		...
		container = create 'ImageLabel' { -- 184 x 494; tab is 47px wide; border is about 4px thick
			...
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://6298138682',-- 313105946 -- 281083812
...
--				cn:disconnect()
	...
	backdrop.Image = 'rbxassetid://'..(self.battleEvent and 6298984173 or 6298983065)
...
function Menu.party:viewSummary(pokemon, ignoreParty, disableMoveSwitching) -- OVH  todo: EV/IV Viewer
	...
	if pokemon.egg then
		...
		if pokemon.shiny then
			create 'ImageLabel' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://413600477', --5217624409,413600477 -- HD NEW
		...
		if pokemon.untradable then -- untradable
			create 'ImageLabel' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://4559783174',
				ImageColor3 = BrickColor.new('Bronze').Color,
				Size = UDim2.new(0.07, 0, 0.07, 0),
				Position = UDim2.new(0.485, 0, 0.38, 0),
				ZIndex = 9, Parent = main,
			}
		end
		...
		local function showDetails(rf, instant)
			...
			if move and not justClose and not instant then
				details = roundedFrame:new {
					...
					create 'ImageLabel' {
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://'..({Status=6223782292,Special=6223783835,Physical=6223776992})[move.category],
	...
	local cancel; cancel = roundedFrame:new {
		...
		MouseButton1Click = function()
			...
			pcall(function() bgFader:Remove() end)
			self.state = 'open'
...
do
	local function triangle(a, b, c)
		...
		local t1 = create 'ImageLabel' {
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://5217627247',--5217628075',
		}
		local t2 = t1:Clone()
		local nodes
...
--			local statnames = {'hp', 'atk','def','spa','spd','spe'}
		...
		cancel.MouseButton1Click:connect(function()
			...
			for _, rf in pairs(rfs) do rf:Remove() end
			rfs = nil
			cancel:Remove()
			fillbar:Remove()
			modalGui:Remove()
...
--		table.insert(rframes, panel)
		...
		local cancel; cancel = _p.RoundedFrame:new {
			...
			MouseButton1Click = function()
				if choose then choose:Remove() end
				cancel:Remove()
				panel:Remove()
				fade:Remove()
				inDetails = false
	...
	for i, m in pairs(r.known) do
		...
		local rf = roundedFrame:new {
			...
			create 'ImageLabel' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://'..({Status=6223782292,Special=6223783835,Physical=6223776992})[m.category],
				SizeConstraint = Enum.SizeConstraint.RelativeYY,
