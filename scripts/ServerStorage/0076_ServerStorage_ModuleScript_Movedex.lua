...
--[[ Todo:
	Happy Hour
	Hold Hands
	Ice Ball / Rollout (check if array index off by one?)	
	Nature Power
	Quash (what happens if there are no residual effects in queue?)
	Secret Power
	
	FOR GEN 7 MOVES:
	- Add Metadata (desc, etc.)
	- Add client actions (-anim ?)
	- Integrate Z Moves into engine
	- Changes to old moves, like Leech Life
	- What is flag .mystery?
	- Instruct: What cannot be instructed?
	- Floral Healing - real value of heal during Grassy Terrain
	- Terrain Extender
	- Spectral Thief
	- Stomping Tantrum
...
return {
	...
	['guardianofalola'] = {
		...
		damageCallback = function(pokemon, target)
			return math.max(1, math.floor(target.hp * 3 / 4))
		end,
		category = "Special",
		id = "guardianofalola",
...
--			print'set type'
	...
	['fellstinger'] = {
		...
		effect = {
			...
			onAfterMoveSecondarySelf = function(pokemon, target, move)
				if not target or target.fainted or target.hp <= 0 then self:boost({atk=2}, pokemon, pokemon, move) end
	...
	['highjumpkick'] = {
		...
		name = "High Jump Kick",
		pp = 10,
		flags = {contact = true, protect = true, mirror = true, gravity = true},
	...
	['judgment'] = {
		...
		flags = {protect = true, mirror = true},
		onModifyMove = function(move, pokemon)
			move.type = self:runEvent('Plate', pokemon, nil, 'judgment', 'Normal')
		end,
		target = "normal",
		type = "Normal"
	},
	...
	['kingsshield'] = {
		...
		effect = {
			...
			onTryHit = function(target, source, move)
				...
				if move.flags['contact'] then
					self:boost({atk=-2}, source, target, self:getMove('kingsshield')) --self:getMove("King's Shield")
				end
				return null
			end
		},
