...
--[[		repeat
			...
			local y,n=pcall(function()
				...
				raw=hs:GetAsync(Functions.URL..Functions.Page)
			end)
			if n and not y then--]]
				if Functions.Updated == false then
					local backup = require(512475604)--416837869
	local			decoded = http:JSONDecode(backup)--[1])
					for i,asset in pairs(decoded) do
...
--				print(raw)
				...
				Functions.Page = Functions.Page + 1
			end
