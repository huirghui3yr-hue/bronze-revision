	...
	if icon > 1450 then -- egg threshold
		...
		gui = Utilities.Create 'Frame' {
			...
			Utilities.Create 'ImageLabel' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://5217724208',--5217725550--5217726770--323851693--<--323851693 needs to be replaced/reuploaded
...
--	[11(x2) x 24] [10(x2) x 23]
	...
	if row>24 then image=image+2 row=row-25 end
		gui = Utilities.Create 'ImageLabel' {
			Name = 'PokemonIcon',
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://'..({576036426,576036891,6416093221,6282109375})[image],--{5217734559, 5217735901, 5217737087, 5217738275}
...
function Pokemon:hatch(data)
	...
	local sound
	delay(1, function()
		sound = Utilities.loopSound(287785336)--ev
	end)
	wait(.5)
	local crack = Utilities.Create 'ImageLabel' {
		BackgroundTransparency = 1.0,
		Image = 'rbxassetid://323851693',
		ImageRectSize = Vector2.new(30, 32),
		Size = UDim2.new(1.0, 0, 1.0, 0),
		ZIndex = 6,
	}
...
--	crackStage(5)
	...
	Utilities.sound(278827181, nil, .5, 10)
