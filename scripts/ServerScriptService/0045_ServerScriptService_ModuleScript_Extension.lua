...
return function(Battle)
	function Battle:runMove(move, pokemon, target, sourceEffect)--, zMove)
		if not sourceEffect and toId(move) ~= 'struggle' then
...
--		self:debug('BeforeMove event result:', ev_res == null and 'null' or ev_res)
		...
		if not lockedMove then
			if not pokemon:deductPP(move, nil, target) and (move.id ~= 'struggle') then
		...
		self:useMove(move, pokemon, target, sourceEffect)
		self:singleEvent('AfterMove', move, nil, pokemon, target, move)
	end
	function Battle:useMove(move, pokemon, target, sourceEffect)
