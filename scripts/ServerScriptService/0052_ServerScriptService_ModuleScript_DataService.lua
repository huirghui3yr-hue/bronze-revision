...
do -- Data Setup
	local cryIds = { 301815648, 301815760, 301815895, 301815944, 301815982,--cr1, cr2, cr3, cr4, cr5 replace/reupload
	                 301816032, 301816086, 301816124, 301816205, 301816307,--cr6, cr7, cr8, cr9, crr10
	                 301816397, 301816525, 301816578, 377125933, 479390302,--cr11, cr12, cr13, 14, altcry15
	                 620418262, 620418671 }--cr16, cr17
...
local cachedBattles = {}
local function completeBattle(num, ch)
	local b = cachedBattles[num]
	if b then
		if b.Name then
			ch.Name = b.Name
		end
...
--	end
	...
	if b.TrainerClass == 'Eclipse Grunt' then
		b.IconId = 5226446131
		b.musicId = {435781330, 435782001}--317469005
		b.musicVolume = 2
	end
...
function Database:getBattle(num)
	if not cachedBattles[num] then
...
--[[				if not baseMap and not loadedExtraMaps then
					loadedExtraMaps = true
					--
					local lm = game:GetService('InsertService'):LoadAsset(game.CreatorId == 1084073 and 664719910 or 664720287)
					for _, chunk in pairs(lm:GetChildren()) do
			...
			if reducedGraphics then -- Mobile Grass Enabled
				...
				if map:FindFirstChild('Grass') then
					...
					map.Grass:ClearAllChildren()--:Remove()
				end
			end
			-- BATTLES
			local battles = {}
				...
				for _, ch in pairs(obj:GetChildren()) do
					if ch.Name == '#Item' then
						...
						if PlayerData then
							...
							pcall(function() tmTag:Remove() end)
						end
						if not ok then
				...
				for id, bd in pairs(d.buildings) do
					...
					if m then
						if id == 'Museum' and reducedGraphics then
							pcall(function() m.Lapras:Remove() end)
						end
						m.Parent = rf
						checkForBattles(m)
						checkForItems(m)
...
--[[						-- Xov_RBLX event
			local bob = map:FindFirstChild('Bob')
			if bob then
				local clausChunks = {'chunk21'}
				local m = game:GetService('Lighting'):GetMinutesAfterMidnight()
				
				if not PlayerData or PlayerData.completedEvents.BeatBob2 or not PlayerData.completedEvents.LearnAboutBob2  
					or (m > 6*60 and m < 20*60) or chunkId ~= clausChunks[PlayerData.userId%3+1] then
            ...
            -- Mew
			if chunkId == 'chunk65' and PlayerData and not PlayerData.completedEvents.Mew then
				storage.Models.Mew:Clone().Parent = map
			end
            -- Volcanion
			if chunkId == 'chunk75' and PlayerData and not PlayerData.completedEvents.Volcanion then
				storage.Models.Volcanion:Clone().Parent = map
			end
			--
			...
			if chunkId == 'chunk14' then
				...
				if keepMonkey ~= map.Pansage then map.Pansage:Remove() end
				if keepMonkey ~= map.Pansear then map.Pansear:Remove() end
				if keepMonkey ~= map.Panpour then map.Panpour:Remove() end
				if keepMonkey then
			...
			delay(30, function()
				pcall(function() rf:Remove() end)
			end)
		elseif kind == 'ChunkReceived' then
			wait(3)
			pcall(function() request[2]:Remove() end)
